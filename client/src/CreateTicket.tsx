...
export default function CreateTicket({ requesterId }: Props) {
  ...
  return <form className="ticket-form" onSubmit={submit} noValidate>
    ...
    <label htmlFor="ticket-summary">Summary <span aria-hidden="true">*</span></label><input id="ticket-summary" value={summary} onChange={(event) => setSummary(event.target.value)} aria-invalid={Boolean(errors.summary)} aria-describedby={errors.summary ? "summary-error" : undefined} />{errors.summary && <span id="summary-error" className="field-error">{errors.summary}</span>}
    <label htmlFor="ticket-description">Description <span aria-hidden="true">*</span></label><textarea id="ticket-description" value={description} onChange={(event) => setDescription(event.target.value)} aria-invalid={Boolean(errors.description)} aria-describedby={errors.description ? "description-error" : undefined} />{errors.description && <span id="description-error" className="field-error">{errors.description}</span>}

      ...
      {panel === "create" && (
        <div className="admin-user-panel" aria-labelledby="create-user-heading">
          ...
          <form className="admin-user-form" onSubmit={submitCreate}>
            <label htmlFor="create-user-name">Name</label><input id="create-user-name" value={createForm.name} onChange={(event) => setCreateForm({ ...createForm, name: event.target.value })} required aria-invalid={Boolean(fieldErrors.name)} aria-describedby={fieldErrors.name ? "create-user-name-error" : undefined} />{fieldErrors.name && <span id="create-user-name-error" className="field-error">{fieldErrors.name}</span>}
            <label htmlFor="create-user-email">Email</label><input id="create-user-email" type="email" value={createForm.email} onChange={(event) => setCreateForm({ ...createForm, email: event.target.value })} required aria-invalid={Boolean(fieldErrors.email)} aria-describedby={fieldErrors.email ? "create-user-email-error" : undefined} />{fieldErrors.email && <span id="create-user-email-error" className="field-error">{fieldErrors.email}</span>}
            ...
            <label htmlFor="create-user-password">Initial password</label><input id="create-user-password" type="password" minLength={12} value={createForm.initialPassword} onChange={(event) => setCreateForm({ ...createForm, initialPassword: event.target.value })} required aria-invalid={Boolean(fieldErrors.initialPassword)} aria-describedby={fieldErrors.initialPassword ? "create-user-password-error" : undefined} />{fieldErrors.initialPassword && <span id="create-user-password-error" className="field-error">{fieldErrors.initialPassword}</span>}
      ...
      {panel === "edit" && selected && (
        <div className="admin-user-panel" aria-labelledby="edit-user-heading">
          ...
          <form className="admin-user-form" onSubmit={submitEdit}>
            <label htmlFor="edit-user-name">Name</label><input id="edit-user-name" value={editForm.name} onChange={(event) => setEditForm({ ...editForm, name: event.target.value })} required aria-invalid={Boolean(fieldErrors.name)} aria-describedby={fieldErrors.name ? "edit-user-name-error" : undefined} />{fieldErrors.name && <span id="edit-user-name-error" className="field-error">{fieldErrors.name}</span>}
            <label htmlFor="edit-user-email">Email</label><input id="edit-user-email" type="email" value={editForm.email} onChange={(event) => setEditForm({ ...editForm, email: event.target.value })} required aria-invalid={Boolean(fieldErrors.email)} aria-describedby={fieldErrors.email ? "edit-user-email-error" : undefined} />{fieldErrors.email && <span id="edit-user-email-error" className="field-error">{fieldErrors.email}</span>}
      ...
      {panel === "reset" && selected && (
        <div className="admin-user-panel" aria-labelledby="reset-user-heading">
          ...
          <form className="admin-user-form" onSubmit={submitReset}>
            <label htmlFor="reset-user-password">New initial password</label><input id="reset-user-password" type="password" minLength={12} value={password} onChange={(event) => setPassword(event.target.value)} required aria-invalid={Boolean(fieldErrors.initialPassword)} aria-describedby={fieldErrors.initialPassword ? "reset-user-password-error" : undefined} />{fieldErrors.initialPassword && <span id="reset-user-password-error" className="field-error">{fieldErrors.initialPassword}</span>}
            {mutationError && <div className="state-callout state-error" role="alert">{mutationError}</div>}

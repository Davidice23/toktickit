...
export default function StaffTicketQueue({ onOpen }: { onOpen: (ticketId: number) => void }) {
  ...
  return <section className="staff-queue" aria-labelledby="staff-queue-heading">
    ...
    {state === "ready" && list && list.data.length > 0 && <>
      <div className="ticket-table-wrap"><table className="ticket-table staff-queue-table"><caption className="visually-hidden">Operational Staff Ticket Queue</caption><thead><tr><th>Ticket Number</th><th>Created / Updated</th><th>Summary</th><th>Requester</th><th>Category</th><th>Requested Priority</th><th>IT Priority</th><th>Status</th><th>Owner</th><th>Action</th></tr></thead><tbody>{list.data.map((ticket) => <tr key={ticket.id}><td data-label="Ticket Number"><strong>{ticket.ticketNumber}</strong></td><td data-label="Created / Updated">{new Date(ticket.createdAt).toLocaleDateString()}<br /><span className="helper-text">{new Date(ticket.updatedAt).toLocaleDateString()}</span></td><td data-label="Summary">{ticket.summary}</td><td data-label="Requester">{ticket.requester.name}<br /><span className="helper-text">{ticket.requester.email}</span></td><td data-label="Category">{ticket.category.name}</td><td data-label="Requested Priority"><span className="status-badge">{ticket.requestedPriority}</span></td><td data-label="IT Priority"><span className="status-badge">{ticket.itPriority ?? "UNASSIGNED"}</span></td><td data-label="Status"><span className="status-badge">{ticket.currentStatus}</span></td><td data-label="Owner">{ticket.owner ? ticket.owner.name : <span className="helper-text">Unassigned</span>}</td><td data-label="Action"><button className="btn btn-secondary-green" type="button" onClick={() => onOpen(ticket.id)}>Open Detail</button></td></tr>)}</tbody></table></div>
